...
export const DialogSearch = (props: IDialogSearchProps) => {
  ...
  const executeSearch = async () => {
    ...
    if (modeSearchContainer) {
      setSearchResults((await MOCKS.getContainers()).slice(0, 5).map((container) => {
        return {
          id: container.id,
      ...
      setSearchResults((await MOCKS.getFiles('fake')).slice(0, 5).map((file) => {
        return {
          id: file.id as string,
          displayName: file.name as string,
          isContainer: false,
          downloadUrl: file.webUrl as string,

...
>(({ className, children, ...props }, ref) => {
  // Create a safe version of children that won't cause rendering issues
  const safeChildren = React.useMemo(() => {
    if (children == null) return []
    return children
  }, [children])

  // Check if there's already a DialogTitle in the children
    ...
    return childrenArray.some(
      child =>
        ...
        (child.type === DialogTitle ||
          (React.isValidElement(child.props?.children) && child.props.children.type === DialogTitle) ||
          (Array.isArray(child.props?.children) && child.props.children.some(c => React.isValidElement(c) && c.type === DialogTitle)))
    )
  }, [safeChildren])

...
module.exports = async function (context, req) {
    ...
    const userPrincipalName = req.headers.userprincipalname;
    ...
    */
    ...
    let rootSite;
    try {
        rootSite = await graph.api('sites/root').get();
    } catch (error) {
        context.res = {
            status: 500,
            body: 'Failed to get root site: ' + error
        };
        return;
    }

    const tenantNameRegex = /^https?:\/\/([^.]+)/;
    const tenantName = rootSite.webUrl.match(tenantNameRegex)[1];
